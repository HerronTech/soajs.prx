	...
	"redirect": function (config, mongo, req, res) {
			...
			if (error || !keyRecord) {
				if (error) {
					...
					return res.json(req.soajs.buildResponse({
						"code": 401,
						"msg": config.errors[401].replace("%envCode%", req.soajs.inputmaskData.__env)
					}));
				}
			}
		...
		function constructProxyRequest(req, envCode, soajsauth, key, requestedRoute, res) {
			req.soajs.awarenessEnv.getHost(envCode, function (host) {
				if (!host) {
					return res.json(req.soajs.buildResponse({"code": 402, "msg": config.errors[402]}));
				}
				soajsCore.registry.loadByEnv({
					...
					if (err) {
						...
						var myUri = (req.headers['x-forwarded-proto'] || "http") + '://' + host + ':' + port + requestedRoute;
