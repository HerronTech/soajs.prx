...
module.exports = {
	"redirect": function (config, mongo, registry, req, res) {
		var requestedRoute = decodeURIComponent(req.query.proxyRoute);
		delete req.soajs.inputmaskData.proxyRoute;
		
		mongo.findOne("tenants", {"code": req.soajs.tenant.code}, function (error, tenantRecord) {
			...
			var extKey = findExtKeyForEnvironment(tenantRecord, req.soajs.inputmaskData.__env.toUpperCase());
